...
> **Abstract.** Sugarchain is the world's fastest PoW blockchain, that has the first Native SegWit (Bech32) built-in by default. Unlike Bitcoin, Sugarchain has no rounding errors when the block reward is halved. Following Nakomoto's vision, it's launch allowed a fair distribution, one-CPU-one-vote.
